...
exports.boyDetail = function (req, res) {
    ...
    validator.check().then(function (matched) {
        ...
        // TODO : ↘ bfc_season_number가 포함된 뷰로 수정.
                    conn.query('select * from sc2_bfc_v_detail where boyid=?',[boykey], function (err, rows) {
                        if(err){
                            conn.release();
                            res.status(200).render('./error', { text:"접속에 문제가 있습니다." } )
                            return;
                        ...
                        if (rows.length>0){  // DB에 있으면...
                            ...
                            res.render('./bfc2_view/boys/boys_detail',{title:"후보상세", boyid:boykey, data:data});
                ...
                res.render('./bfc2_view/boys/boys_detail',{title:"후보상세", boyid:boykey, data:JSON.parse(data)});
                return;
            }

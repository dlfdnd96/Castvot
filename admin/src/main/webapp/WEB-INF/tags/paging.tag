...
<%@ attribute name="candidate" type="com.castvot.admin.vo.candidate.CandidateParam" required="true" %>
		...
		<c:choose>
			<c:when test="${value.pageNo gt value.prevPageNo}">
				<li class="paginate_button page-item previous" id="bootstrap-data-table_previous">
					<a href="?bfcSeasonNumber=${candidate.bfcSeasonNumber}&pageNo=${value.prevPageNo}&searchType=${value.searchType}&searchValue=${value.searchValue}&sortType=${value.sortType}&sortValue=${value.sortValue}&statusType=${value.statusType != null ? value.statusType : ''}" aria-controls="bootstrap-data-table" class="page-link">Previous</a>
				</li>
			</c:when>
			<c:otherwise>

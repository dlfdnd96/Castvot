...
exports.updateTodayBj = function (req, res) {
    ...
    cache.get(rediskey,function(err, data){
        ...
        if(data === null){ // 캐쉬 없을때
            ...
            dbpool.getConnection(function(err, conn) {
                ...
                let yes_val;
                let todayBest = new Array;
                let len;

                conn.query('SELECT * FROM bjc_today_total', function (err, rows) {
                    if(err){
                        conn.release();
                ...
                conn.query('SELECT * FROM bjc_v_main ORDER BY total DESC, name LIMIT ?', [config.main_max], function (err, rows) {
                    ...
                    for(let i = 0; i < len; i++){
                        todayBest.push(rows[i].total - yes_val.yes_total);
                    }
